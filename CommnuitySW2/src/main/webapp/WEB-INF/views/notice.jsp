...
<body>
	...
	<div class="container" style="margin:50px; margin-left:300px;">
		<h2 id="loginBoxTitle" style="width:100%; text-align:center;">공지사항</h2>
	...
	<script>
		...
		db.collection('noticeboard').get().then((test)=>{
			test.forEach((doc)=>{
				...
				html += '<tr>';
				html += '<td><a href="noticeview?content=' +doc.data().content + '&date=' +doc.data().date +  '&username=' +doc.data().username +  '&viewcnt=' +doc.data().viewcnt +  '&title='+doc.data().title + ' ">' + doc.data().title+'</td>';
		...
		$(document).on('click','#gowrite',function(){
			...
			firebase.auth().onAuthStateChanged(function(user) {
	  			...
	  			if (user && manager == "1") {
					console.log(manager);
					window.location.href="/controller/noticewrite"			
	  			}else{
					alert("관리자만 등록 가능합니다");
				}
			});
