...
<body>
	...
	<div class="container" style="margin: 50px; margin-left: 300px;">
		...
		<input type="button" id="gowrite" value="신규등록" class="pull-right"
			style="align-items: center; margin-top: 10px; background-color: black; color: white; border-radius: 5px; padding: 10px;">
	</div>
	...
	<script>
		...
		// Concrete strategy for logged-in state
		class LoggedInClickStrategy extends ClickStrategy {
  			execute() {
    			window.location.href = "/controller/write";
  			}
		}

		// Concrete strategy for logged-out state
		class LoggedOutClickStrategy extends ClickStrategy {
  			execute() {
    			alert("로그인이후 신규 등록이 가능합니다.");
  			}
		}
		...
		const loggedInStrategy = new LoggedInClickStrategy();
		const loggedOutStrategy = new LoggedOutClickStrategy();

		// Set the initial strategy based on the login state
		firebase.auth().onAuthStateChanged(function(user) {
  			if (user) {
    			clickHandler.setStrategy(loggedInStrategy);
  			} else {
   				 clickHandler.setStrategy(loggedOutStrategy);
  			}
		});
