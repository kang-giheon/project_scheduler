...
<body>
	...
	<div class="container" style="margin:50px; margin-left:300px;">
		...
		<div>
			...
			<button onclick="location.href = 'notice'" class="pull-right" style="align-items:center; background-color:black;
	...
	function joinDB(title, content) {
		firebaseEmailAuth.onAuthStateChanged( async (noticeboard) => {
			
			var viewcnt = 0;
			var data = {username:nowusername,title:title, content:content,date:currentTime,viewcnt:viewcnt,uid:nowUid};
			var bid = 0;
			const res = await db.collection('noticeboard').doc(noticeboard.bid).set(data);
			window.location.href="/controller/notice"
			})
	}
	</script>
